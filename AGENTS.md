# VQ2 Campaign — session bootstrap (2026-07-16)

**ENVIRONMENT CHECK FIRST:** this repo now runs in TWO places. If the hostname
/ user is Alex's laptop, the laptop sections below apply verbatim. If you are
on a **Vagon cloud machine**, read `docs/vagon-bootstrap.md` FIRST — paths,
GPU, WSL availability, and the deploy target differ; the judge laws, sim state
machine, and discipline sections below still apply everywhere.

## 2026-07-16 PIVOT (supersedes the survey plan)
- The autonomous-survey/mapping plan (docs/superpowers 2026-07-16 specs/plans)
  was STOPPED at 0% implementation. Do not resume it without Alex's say-so.
- Compute is moving to **Vagon** (cloud Windows desktop, bigger GPU — the 4 GB
  GateNet/DPVO/sim co-residency wall may not exist there; verify, don't assume).
- **Janahan supplies a 6-gate course-map JSON** (contract:
  `docs/vq2-map-json-contract.md`; loader: `vq2/map_ingest.py` + tests).
  vq2wp.py still has gates HARDCODED (G1_AP/G2_W/GATES_W ~lines 103-112);
  wiring MAP_JSON into it is the open integration step.
- Full design/debate record: `docs/handoff/dpvo_scale_review.md` (scale
  non-determinism, tick=aperture-not-center, external Sim3 wrapper, survey
  rationale). DPVO latency work: fast config = patches 24 + 320x180 +
  REMOVAL/OPT window 12/6 -> 171 ms sim-closed vs 300 ms stock (observe-only;
  scale is per-session, see docs/handoff + memory).

You are (when on the laptop) running LOCALLY on Alex's Windows laptop (the
flight machine). Everything below was established over 07-05..07-12; trust it,
don't re-derive. Mission: judge ticks through gates 1 and 2 of the AI-GP VQ2
qualifier (TICKS=2). Sim: AI-GP v1.0.3385, `C:\Users\alexj\Documents\AI-GP
Simulator v1.0.3385\FlightSim.exe`. ~14 days remain on the qualifier clock.
