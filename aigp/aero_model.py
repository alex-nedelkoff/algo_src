...
def moment_features(v_body, omega) -> np.ndarray:
    ...
    full moment fit stays a least-squares solve:
      - axis 0 (roll):  w_x col <- v_y  (lateral-velocity roll coupling)
      - axis 1 (pitch): w_y col <- v_x  (axial-velocity pitch; tail-first destabilizer)
      - axis 2 (yaw):   w_z col <- v_x  (axial-velocity yaw; tail-first destabilizer)
    """
    v = np.asarray(v_body, float)
    ...
    phi[0, 3] = v[1]  # roll  <- v_y   (w_x column)
    phi[1, 4] = v[0]  # pitch <- v_x   (w_y column)
    phi[2, 5] = v[0]  # yaw   <- v_x   (w_z column)
    return phi
