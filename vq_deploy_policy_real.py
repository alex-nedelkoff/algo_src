...
LATFLIP = "--nolatflip" not in sys.argv   # DEFAULT ON (proven on teacher 06-16: mirror 0/6, latflip captures). policy lateral-sign fix (deploy frame is a y-mirror of the training-env frame):
# negate the roll-rate OUTPUT (the policy's analog of the teacher's a[1] latflip). The full obs+output relabel is a
# no-op (relabel preserves physics), so this is a single-axis flip. A/B vs --obsflip live with the retrained policy.
...
YAWPI = "--yawpi" in sys.argv   # offset yaw-err obs o[8] by pi (camera-forward A/B; training o[8]~pi)
...
def build_obs(pe, ve, qe, ome, gi, prev_act, hist):
    ...
    if YAWPI: o[8] = wrap(o[8] + np.pi)   # yaw-convention fix: deploy o[8] ~pi off training (camera reversal)
    o[9:12] = ome; o[12:16] = MOTOR_NORM; o[16:20] = prev_act
    dg = gates[gi1] - gate
    o[20:22] = rotxy(dg[:2], gy); o[22] = dg[2]; o[23] = wrap(gyaws[gi1]-gy); o[24] = 2.72; o[25] = 2.72
    o[26] = ARENA
...
while time.time()-t0 < MAX_T and gi < NG:
    ...
    try:
        ...
        if _cseq > prev_cseq:
            ...
            if _cobj is not None: print(f"  [COLL id={_cobj.get('id')} imp={_cobj.get('impulse', 0):.2f}] t={t:.1f}", flush=True)
    except Exception:
        pass
