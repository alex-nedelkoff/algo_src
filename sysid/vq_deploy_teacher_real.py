...
COR-136 RACE CONFIGS (judge-confirmed 6/6, 0 collisions, camera-forward, waits for the GO flag):
  FAST  (~21.35s, v14.4): --vdes 26 --vgate 11 --tiltbudget 64 --kdxy 12 --kdatt 1.6 --zvd
        rides the >45deg rate-ring at the tightest gate (actual tilt spikes ~73, pin-recovery catches it -- repeatable but marginal).
  ROBUST(~22.2s,  v10.2): --vdes 24 --vgate 20 --tiltbudget 60 --kdxy 10 --zvd   (actual tilt ~43, well under the ring)
  Wall: tiltbudget >=66 detonates the ring (true_tilt 103, tumble). canon --vdes 8 = 32.1s.
  *** DO NOT pass --earlystart: flying during the 3-count countdown is a FALSE START (DSQ). ***
"""
...
GATE_R = argf("--gater", 2.0); ABORT_TILT = 100.0; LEVEL_T = argf("--levelt", 0.0); GRACE = LEVEL_T + 1.5
EARLYSTART = "--earlystart" in sys.argv   # COR-136: *** DSQ (false start) -- DO NOT USE in a real race. *** Flies during the 3-count countdown. DEFAULT OFF: wait for the countdown/race-live flag, then go.
...
def fresh_start():
    ...
    while time.time()-t < 30:
        idle(); d = s.get_drone()
        if EARLYSTART:   # go during the countdown once the drone has teleported to the fresh spawn (consistent with raw gate0, settled) -- NOT the stale post-finish frame
            _g = s.get_gates()
            if d is not None and _g and np.linalg.norm(d.vel_ned) < 3.0 and np.linalg.norm(np.array(_g[0].pos_ned) - d.pos_ned) < 50.0: return True
        elif s.get_race_live() and d is not None and np.linalg.norm(d.vel_ned) < 3.0: return True
        time.sleep(0.02)
    return False
...
_ok = False
for _att in range(4):
    if fresh_start(): _ok = True; break
    print(f"  fresh_start retry {_att+1} (re-reset)...", flush=True); time.sleep(1.0)
assert _ok, "not live"
...
traj = GateTrajectory(spline_wps, v_cruise=VDES,
                      tilt_budget_deg=argf("--tiltbudget", 35.0), c_drag=0.057, margin=0.6, vz_max=VZMAX)   # COR-136: real-course planner tilt now configurable (was hardcoded 35); raise toward the ~45deg ring for speed
...
while time.time()-t0 < MAX_T:   # run on the REAL judge (below), NOT the geometric gi (which trips ~2m early and cut gate5 short)
    ...
    if dist < GATE_R and gi < NG and (gi > 0 or s.get_race_live()):   # COR-136: don't advance the geometric counter past gate0 until the timer is live (earlystart flies during the countdown)
