...
SIGN_S = argf("--signs", -1.0); SIGN_Y = argf("--signy", 1.0)
K_YAWVIS = argf("--kyaw", 0.8)            # visual yaw gain: ex (norm. pixel) -> yaw_ref rate
...
def main():
    ...
    try:
        while time.time() - t0 < DURATION:
            ...
            # SLEWED speed target + gentle brake: stepping 1.2<->0.5 on detection flicker excited
            # the underdamped attitude resonance (attempt-4/5 pump, tilt 5<->50). Slow is fine here.
            fwd_tgt = 0.5 if blind else FWD
            fwd_cur_v = getattr(main, "_fwd_cur", FWD)
            fwd_cur_v += float(np.clip(fwd_tgt - fwd_cur_v, -0.3 * dt, 0.3 * dt)); main._fwd_cur = fwd_cur_v
            a_al = float(np.clip(KD_AL * (fwd_cur_v - v_al), -0.8, AL_MAX))
            moving = v_al > 0.3
            ...
            if locked:                                           # ballistic: hold heading + elevation track
                ...
                # sweep ONLY while moving + not in the first 10 s: near-hover yaw rotation is the
                # lethal regime (deploy3; attempt-3/4 scan-tumbles both happened near-hover).
                if moving and t > 10.0 and (now - last_det_t) > SCAN_AFTER:
